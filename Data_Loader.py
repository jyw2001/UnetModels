...
# 从给定目录中加载图像和标签，并对它们应用个体化的转换，确保图像和标签的转换同步。最终返回转换后的图像和标签。
class Images_Dataset_folder(torch.utils.data.Dataset):
    ...
    # 如果没有提供转换函数transformI和transformM，则使用默认的转换函数。
    ...
    def __getitem__(self, i):
        ...
        seed=np.random.randint(0,2**31-1) # make a seed with numpy generator
